...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
    ...
    // Verifica se o caractere é minúsculo ou maiúsculo
    if (c >= 'a' && c <= 'z') {
        index = c - 'a'; // Se for minúsculo, calcula o índice para letras a-z
    } else if (c >= 'A' && c <= 'Z') {
        index = c - 'A' + 26; // Se for maiúsculo, calcula o índice para letras A-Z (de 26 até 51)
    } else if (c >= '0' && c <= '9') {
        index = c - '0' + 52; // Se for número, calcula o índice para os números 0-9 (de 52 até 61)
    } else {
        return; // Caso o caractere não seja válido, não faz nada
    }

    // Desenha cada coluna do caractere
    for (int i = 0; i < 8; i++) {
        uint8_t line = font[index * 8 + i]; // Pega a linha do caractere
        for (int j = 0; j < 8; j++) {
            if (line & (1 << j)) {
                // Desenha o pixel se o bit correspondente estiver ativado
...
#define CHAR_SPACING 1  // Define o espaçamento entre caracteres

void ssd1306_draw_string(ssd1306_t *ssd, const char *str, uint8_t x, uint8_t y)
{
    while (*str)
    {
        if (x + 8 > ssd->width) // Verifica se precisa quebrar a linha
        {
            x = 0;
            y += 8;
        }

        if (y + 8 > ssd->height) // Impede que caracteres saiam da tela
        {
            return;
        }

        ssd1306_draw_char(ssd, *str++, x, y);  // Desenha o caractere
        x += 8 + CHAR_SPACING; // Move o cursor para o próximo caractere
    }
}